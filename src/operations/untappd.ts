...
// Services
import UntappdService, { beerSearchRes, beerInfoRes, beerSearchBeer } from '../services/untappd'

const createBeerAttachment = (theBeer: beerSearchBeer, theBeerRating: string, theBeerNumRatings: number) => {
...
const testCreateBeerAttachment = (theBeer: beerSearchBeer) => {
	const thisAttachment = {
		color: '#ffcc00',
...
const processBeer = async (beerToProcess: beerSearchBeer) => {
	const beerID = beerToProcess.beer.bid

	try {
		const beerInfoRes = await UntappdService.beerInfo(beerID)
		const beerInfoBody: beerInfoRes = beerInfoRes.response
...
const createSharedAttachment = async (beerID: number) => {
	try {
		const beerInfoRes = await UntappdService.beerInfo(beerID)
		const beerInfoBody: beerInfoRes = beerInfoRes.response
...
const processSearchResults = async (beerName: string) => {
	let res
	let body: beerSearchRes
	let beers: beerSearchBeer[] = []
	let numBeers: number = 0
